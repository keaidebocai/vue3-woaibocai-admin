...
const fetchData = async () => {
  const { data } = await FindAllPage(
    paramPages.value.current,
    paramPages.value.size,
    queryCriteria.value
  )
...
const Delete = async (row) => {
  ElMessageBox.confirm('此操作将永久删除该记录, 是否继续?', 'Warning', {
    ...
    await DeleteById(row.id, row.url)
  ...
  <div>
    <el-form :inline="true" :model="queryCriteria" class="demo-form-inline">
      ...
      <el-form-item label="是否置顶:" style="width: 180px">
        <el-select v-model="queryCriteria.isTop" placeholder="请选择" clearable>
      ...
      <el-form-item label="是否允许评论:" style="width: 180px">
        <el-select v-model="queryCriteria.isCommont" placeholder="请选择" clearable>
          <el-option label="全部" value="" />
      ...
      <el-form-item label="是否发布:" style="width: 180px">
        <el-select v-model="queryCriteria.status" placeholder="请选择" clearable>
          <el-option label="全部" value="" />
      ...
      <el-form-item label="排序根据" style="width: 180px">
        <el-select v-model="queryCriteria.orderBy" placeholder="从高到低" clearable>
    ...
    <el-table :data="blogArticle" style="width: 100%">
      ...
      <el-table-column fixed="right" label="操作" width="220">
        <template #default="scope">
          <el-button type="info" size="small" @click="ReviseClick(scope.row.id)"
            >修改文章</el-button
          >
