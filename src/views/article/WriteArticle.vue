...
import { UpdateArticle, UpdateArticleData, WriteArticle, UserOnUploadImage } from '@/api/article'
...
const onUploadImg = async (files: File[], callback: (result: string[]) => void) => {
...
const goOnWeb = async () => {
  if (write.value.tags.length == 0) {
    ElMessage.error('tag忘选啦！')
    return
  }
  if (write.value.thumbnail == '') {
    ElMessage.error('thumbnail没传')
    return
  }
...
<template>
  <div>
    <div>
      <el-form :inline="true" style="width: 300">
        ...
        <el-form-item label="是否置顶:" style="width: 180px">
          <el-select v-model="write.isTop" placeholder="请选择" clearable>
        ...
        <el-form-item label="是否允许评论:" style="width: 180px">
          <el-select v-model="write.isCommont" placeholder="请选择" clearable>
        ...
        <el-form-item label="是否发布:" style="width: 180px">
          <el-select v-model="write.status" placeholder="请选择" clearable>
      ...
      <el-form :inline="true">
        ...
        <el-form-item label="url:">
          <el-input placeholder="url" v-model="write.url" type="textarea" />
        </el-form-item>
        <!-- 图片上传 -->
        <el-upload
    ...
    <MdEditor v-model="write.content" @onUploadImg="onUploadImg" />
